...
mutable struct microstate # holds the variables of the actual current or proposed state
    ...
    r_box::Vector{SVector{3,Float64}}  # position of all N particles (3xN matrix)
    r_frac_box::SVector{3,Float64} # position of fractional particle
end
...
function copy_microstate!(dest::microstate, src::microstate)
    ...
    # Handle potentially different sizes - resize or reassign
    if length(dest.r_box) != length(src.r_box)
        dest.r_box = copy(src.r_box)  # Allocate new array with correct size
    else
...
mutable struct SimCache
    ...
    =#
    ζ_Svec::SVector{3,Float64} # created for use in translate_by_random_vector() to store the new random vector 
    ζ_idx::Int64 # created for use in translation_move! to pick random atom to move 
    ri_proposed_box::SVector{3,Float64} # createed for use in translation_move!() to store new proposed location of fractional or normal particle 
    μ_prop::microstate # used for proposing moves in λ_move!()
end

function init_cache()::SimCache
    ζ_Svec = @SVector zeros(3)
    ζ_idx = Int64(0)
    ri_proposed_box = @SVector zeros(3)
    μ_prop = microstate(
                        0,
                        0,
                        Vector{SVector{3,Float64}}(),   # empty vector is correct
                        @SVector [0.0, 0.0, 0.0] )
...
function load_configuration(filename::String)::Tuple{Int,Float64,Vector{SVector{3,Float64}}}
...
@inline function matrix_to_svec_positions(r_box::AbstractMatrix{Float64})::Vector{SVector{3,Float64}}
    @assert size(r_box, 1) == 3
    N = size(r_box, 2)

    r = Vector{SVector{3,Float64}}(undef, N)
    @inbounds for j in 1:N
        r[j] = SVector{3,Float64}(
            r_box[1, j],
            r_box[2, j],
            r_box[3, j],
        )
...
function init_microstate(;filename::String,λ::Int=0, r_frac_box::SVector{3,Float64}=@SVector [0.0, 0.0, 0.0] )::microstate
    ...
    r_box = [pbc_wrap(r) for r in r_box] # periodic conditions, forces any atoms outside the box to wrap around and go back in
    μstate = microstate(N,λ,r_box,r_frac_box)
    return(μstate)
end # init microstate

@inline pbc_wrap(r::SVector{3,Float64}) = r - SVector(round(r[1]), round(r[2]), round(r[3]))
