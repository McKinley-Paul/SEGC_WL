...
function run_simulation_benchmark()
    ...
    Profile.Allocs.@profile sample_rate=1 run_simulation!(sim,μstate,wl) 
    # @time output:  16.137074 seconds (533.01 M allocations: 14.336 GiB, 5.20% gc time)
    println(wl.logf, " ", wl.iters)
end

function warntype_translation_move()
    ...
    @code_warntype translation_move!(sim,μ,wl) # this looks pretty good and only shows red for AbstractRNG
end
